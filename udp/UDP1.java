...
byte[] data = new byte[4];byte[] data1 = new byte[4];byte[] data2 = new byte[4];
  ...
  System.out.println("Type some text if u want to Quit type 'exit'.");
   Random rand = new Random();
    ...
    data[0]=(byte)(n);data[1]=9;data[2]=0;data[3]=1;
for(int i=0;i<4;i++)
   ...
   if(((int)buf[0])==0)
{
  ...
  buf[1]=(byte)a;
   ...
   buf = str5.getBytes();
    clientsocket.send(new DatagramPacket(buf,str5.length(),ia,sport));
   String str6 = new String(data2);
   buf = str6.getBytes();
   clientsocket.send(new DatagramPacket(buf,str6.length(),ia,333));
}
